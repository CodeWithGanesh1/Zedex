...
const showToast = (product, variant) => {
    setToast({ product, variant })
    ...
    const displayImages = (activeVariant?.images && activeVariant.images.length > 0)
        ...
        : (product.images && product.images.length > 0 ? product.images : [ { url: '/zedex_image.avif' } ]);
                            ...
                            <div className="flex flex-col gap-4 mt-auto">
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        showToast(product, activeVariant)
                                    }}
                                >
                                    Add to Cart
                                </button>
...
{toast && (
    <div
        className="fixed top-6 right-6 z-50 flex items-center gap-4 p-4 shadow-lg transition-all duration-500"
        style={{
    ...
    >
        {/* Product Image */}
        <div className="w-14 h-16 flex-shrink-0 overflow-hidden" style={{ backgroundColor: '#f5f3f0' }}>
            {toast.variant?.images?.[0]?.url || toast.product?.images?.[0]?.url ? (
                <img
                    src={toast.variant?.images?.[0]?.url || toast.product?.images?.[0]?.url}
        ...
        <div className="flex-grow min-w-0 space-y-1">
            <p className="text-[10px] uppercase tracking-widest" style={{ color: '#C9A96E' }}>
                Added to Cart
            </p>
            <p className="text-sm truncate" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#1b1c1a' }}>
                {toast.product?.title}
            </p>
            {toast.variant?.attributes && (
                <p className="text-[10px] uppercase tracking-wider" style={{ color: '#7A6E63' }}>
                    {Object.values(toast.variant.attributes).join(' / ')}
                </p>
            )}
        </div>
        ...
        <Link
            to="/cart"
            className="flex-shrink-0 py-2 px-4 text-[10px] uppercase tracking-widest transition-all duration-300"
            style={{
                backgroundColor: '#1b1c1a',
                color: '#fbf9f6',
            }}
